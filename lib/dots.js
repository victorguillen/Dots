...
class Game {
  constructor(ctx1) {
    ...
    this.restart();
    this.timer();
    this.snd = new Audio("lib/drop.wav");
    // this.drop = drop;
    this.pause = true;
    this.connections = [];
    this.pointsCount = 0;
    ...
    this.time = 60;
    this.mute = true;
    this.buttons = [[90, 530, 36], [234, 530, 36]]; // x, y, radius play/pause, restart
  }
  ...
  drawButtons() {
    if(this.pause) {
      ...
      this.ctx.moveTo(76, 508);
      this.ctx.lineTo(76, 552);
      this.ctx.lineTo(112, 530);
      this.ctx.lineTo(76, 508);
      this.ctx.stroke();
    } else {
      this.ctx.beginPath();
      this.ctx.arc(this.buttons[0][0], this.buttons[0][1], this.buttons[0][2], 0, Math.PI*2, false);
      ...
      this.ctx.strokeStyle = "#FFFFFF";
      this.ctx.lineWidth = 1;
      this.ctx.moveTo(78, 512);
      this.ctx.lineTo(78, 548);
      this.ctx.stroke();

      this.ctx.beginPath();
      this.ctx.strokeStyle = "#FFFFFF";
      this.ctx.lineWidth = 1;
      this.ctx.moveTo(102, 512);
      this.ctx.lineTo(102, 548);
      this.ctx.stroke();
    }
    this.ctx.beginPath();
    ...
    this.ctx.beginPath();
    this.ctx.arc(234, 530, 24, 2*Math.PI/4, 6*Math.PI/4, false);
    this.ctx.strokeStyle = "#FFFFFF";
    this.ctx.lineTo(222, 518);
    this.ctx.stroke();

    this.ctx.beginPath();
    this.ctx.arc(234, 530, 24, 0, 6*Math.PI/4, false);
    this.ctx.strokeStyle = "#FFFFFF";
    this.ctx.lineTo(218, 502);
    this.ctx.stroke();
  }
